...
public:
	...
	bool Construct(EBindFlags bindFlags=EBindFlags::B_VERTEXBUFFER, EUsageFlags usage=EUsageFlags::U_IMMUTABLE, bool deleteData = true)
	{
		...
		if (!BufferInitialized || usage != EUsageFlags::U_DYNAMIC)
		{
			...
			if (!Buffer->Init(&TempDataStorage[0],
				...
				bindFlags,
				usage,
				usage == EUsageFlags::U_DYNAMIC ? ECPUAccessFlags::CA_WRITE : ECPUAccessFlags::CA_NONE))
				return false;
