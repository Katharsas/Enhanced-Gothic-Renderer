...
bool RLineRenderer::InitResources()
{
	...
	LEB_R(LineBuffer->Init(nullptr, sizeof(LineVertex) * NUM_START_LINE_VERTICES, sizeof(LineVertex), B_VERTEXBUFFER, U_DYNAMIC, CA_WRITE, "LineBuffer"));

	LineCB = REngine::ResourceCache->CreateResource<RBuffer>();
	LEB_R(LineCB->Init(nullptr, sizeof(LineConstantBuffer), sizeof(LineConstantBuffer), B_CONSTANTBUFFER, U_DYNAMIC, CA_WRITE, "LineCB"));
...
void RLineRenderer::PlotNumbers(const std::vector<float>& values, const float3& location, const float3& direction, float distance, float heightScale, const float4& color)
{
	for(unsigned int i=1;i<values.size();i++)
	{
		AddLine(LineVertex(location + (direction * (float)(i-1) * distance) + float3(0,0,values[i-1]*heightScale), color), 
				LineVertex(location + (direction * (float)i * distance) + float3(0,0,values[i]*heightScale), color));
	}
}
...
{
	float3 u = location; u.z += size;
	float3 d = location; d.z -= size;

	float3 r = location; r.x += size;
	float3 l = location; l.x -= size;

	float3 b = location; b.y += size;
	float3 f = location; f.y -= size;
...
void RLineRenderer::AddAABB(const float3& location, float halfSize, const float4& color)
{
	...
	AddLine(LineVertex(float3(location.x - halfSize, location.y - halfSize, location.z - halfSize), color), LineVertex(float3(location.x + halfSize, location.y - halfSize, location.z - halfSize), color));
...
void RLineRenderer::AddRingZ(const float3& location, float size, const float4& color, int res)
{
	...
	for(int i=0;i<res;i++)
	{
		points.push_back(float3(size * sinf(step * i) + location.x, size * cosf(step * i) + location.y, location.z));
	}
